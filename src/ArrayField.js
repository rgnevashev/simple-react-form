  ...
  renderChildrenItem({ children, index }) {
    ...
    return (
      <div className={this.props.childrenClassName} key={`${this.props.fieldName}.${index}`} data-id={`${id}`}>
        <div className="col-sm-11">{this.renderChildrenItemWithContext({ children, index })}</div>
  ...
  render() {
    ...
    return (
      <WrapperField {...this.props}>
        <div className={classNames('sortable-container', parentClassName)}>{this.renderChildren()}</div>
        <div>
          {showAddButton &&
            !disabled && (
